...
from app.services.research.prompts.summarizer_prompt import SUMMARIZER_PROMPT
...
class SummarizerAgent:
    ...
    def summarize(
        ...
        if financial_data:
            trimmed_fin = {k: v for k, v in financial_data.items() if k != "raw_response"}
            sections.append(f"## FINANCIAL INTELLIGENCE\n{_truncate(trimmed_fin, 1200)}")
        else:
            sections.append("## FINANCIAL INTELLIGENCE\nNot available.")

        if people_data:
            trimmed_people = {k: v for k, v in people_data.items() if k != "raw_response"}
            sections.append(f"## PEOPLE & DECISION MAKERS\n{_truncate(trimmed_people, 1500)}")
        else:
        ...
        if competitive_data:
            trimmed_comp = {k: v for k, v in competitive_data.items() if k != "raw_response"}
