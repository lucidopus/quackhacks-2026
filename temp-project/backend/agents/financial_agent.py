...
from backend.prompts.financial_prompt import FINANCIAL_PROMPT
...
class FinancialAgent:
    ...
    def analyze(
        ...
        """
        Research financial health and metrics.

        Args:
            company_name: Company name
            crunchbase_url: Optional Crunchbase URL for funding data
            company_type: 'public' / 'private' / 'startup'

        Returns:
            Dict with revenue, funding, employee count, market position, etc.
        """
        crunchbase_hint = (
            f"\nCrunchbase page: {crunchbase_url}" if crunchbase_url else ""
        )
